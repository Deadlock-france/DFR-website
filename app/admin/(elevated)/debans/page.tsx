...
import { listDebanRequestsAdmin } from "@/lib/admin/deban";
...
export default async function AdminDebansPage({
  searchParams,
}: {
  ...
  const rows = await listDebanRequestsAdmin(statusFilter);
  const pendingCount = (
    statusFilter === "pending"
      ? rows
      : await listDebanRequestsAdmin("pending")
  ).length;
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-colus text-3xl tracking-wide">Demandes de déban</h1>
            ...
            >
              <div className="min-w-0">
                <p className="font-medium text-foreground">
                  {row.applicant_label ?? row.discord_id}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Ban : {row.ban_reason || "—"} · {debanStatusLabel(row.status)}{" "}
                  · {formatDt(row.created_at)}
                </p>
              </div>
              <Link
                href={`/admin/debans/${row.id}`}
