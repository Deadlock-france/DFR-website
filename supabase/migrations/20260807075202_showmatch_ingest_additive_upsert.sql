-- Additive upsert ingest:
-- - no snapshot DELETE
-- - series / teams / games upserted by stable keys
-- - roster: replace only when teams[].players is present; otherwise leave untouched
-- - game participants: replace only when games[].players is non-empty; empty/absent = keep
  ...
  loop
    ...
    loop
      ...
      values (
        ...
        case when v_game ? 'duration_seconds'
      ...
      on conflict (series_id, game_number) do update
      set
        deadlock_match_id = case
          when v_game ? 'deadlock_match_id' then excluded.deadlock_match_id
          else showmatch_games.deadlock_match_id
        end,
        started_at = case
          when v_game ? 'started_at' then excluded.started_at
          else showmatch_games.started_at
        end,
        duration_seconds = case
          when v_game ? 'duration_seconds' then excluded.duration_seconds
          else showmatch_games.duration_seconds
        end,
        total_kills = case
          when v_game ? 'total_kills' then excluded.total_kills
          else showmatch_games.total_kills
        end,
        total_souls = case
          when v_game ? 'total_souls' then excluded.total_souls
          else showmatch_games.total_souls
        end,
        winner_team_id = case
          when v_winner_team_key is not null then excluded.winner_team_id
          else showmatch_games.winner_team_id
        end,
        side_mapping_source = case
          when v_game ? 'side_mapping_source' then excluded.side_mapping_source
          else showmatch_games.side_mapping_source
        end,
        mvp_rule = case
          when v_game ? 'mvp_rule' then excluded.mvp_rule
          else showmatch_games.mvp_rule
        end,
        updated_at = now()
...
comment on function public.ingest_showmatch(jsonb) is
  'Bot Discord: additive upsert. Extra fields ignored. Roster replaced only when teams[].players is present; game players replaced only when non-empty.';
